<template>
    <div
        ...
        <button v-for="(tab, idx) in store.windInfoData" :id="`item_${tab.parameter_name}`" :key="idx" :class="[
            itemClass,
            localeValue === tab.parameter_name
                ? activeItemsClass
                : 'text-dark hover:text-dark/60',
        ]" class="py-4 transition-all duration-200 w-max text-base font-medium z-10"
...
onMounted(async () => {
    ...
    const item = document.getElementById(`item_${defaultParam}`) as HTMLButtonElement;
    if (item) {
        active.value = {
