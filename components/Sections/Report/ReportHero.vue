...
const fetchReports = async () => {
  ...
  try {
    const response = await api.$get(
      `${locale.value}/api/main/report/?page=${currentPage.value}&page_size=${pageSize}`
    )
...
<template>
  <section class="bg-gray-200 pt-[52px] pb-[84px]">
    <div class="container">
      ...
      <div
        v-if="loading"
        class="flex justify-center items-center mt-8 h-screen"
      >
      ...
      <div
        v-else
        class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[20px] mt-8"
      >
        <ReportCard
          v-for="(report, key) in reports"
          :key
          :title="report?.title"
          :size="(report?.file_size / 1024 / 1024).toFixed(1) + ' MB'"
          :format="report?.file_type.toUpperCase()"
      ...
      <!-- Пагинация -->
      <div
        class="flex items-center justify-end mt-[22px]"
